...
13. As the owner, I want to define HOME and NEARBY as lists of microdistrict names (with aliases and typo tolerance), so that I don't need coordinates or a map.
...
**Profiles as data.** `profiles/<channel>.yaml`: `channel, weight, language, threads_by_reply, noise_patterns[], type_vocab{}, place_aliases{}, examples[]` where each example is `{text, type, stage, places, count}` and doubles as a test. `add-channel` writes a draft and a coverage report; a human edits and commits. `review` writes proposed diffs to a review file, never to the profile.
...
- Open decisions left to the owner, with defaults: drone URGENT cadence during a multi-hour raid (default: 5-min cooldown), NEARBY ring membership (default from BEHAVIOR.md), personal vs dedicated Telegram account (default: dedicated), initial channel weights (default: the values listed under Data source).
