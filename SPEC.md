...
5. As a household member, I want a terse `ЦІЛЬ` with no target to arrive as WATCH — whenever there is ballistic context: a declared threat within 15 min, a live ballistic event, or a Kyiv place named in the launch itself — and be promoted to URGENT the moment any channel names a Kyiv place, so that speed and precision are both preserved. A bare `ЦІЛЬ` with no such context is stored, never pushed (`docs/adr/0001-pending-launch-lifecycle.md`).
...
7. As a household member, I want the alarm to re-sound only on tier promotion, a new launch after ≥2 min, a count jump, a drone returning to the home set after being reported in the ring, or a new event after a ≥10 min quiet gap, so that repeated chatter about the same drone doesn't keep ringing (#16).
...
33. As the owner, I want the drone cooldown per tier (5/10/20 min), the ballistic resound gap (2 min) and the global URGENT floor (2 min) to be configuration, so that I can tune the 28 Aug "nine alarms in seven hours" experience without code changes.
...
RESOUND  new launch form while event active and ≥ 2 min since last sound;
         ...
         and never within `urgent_floor_min` of any other event's URGENT sound (#16)
...
**Drone events.** Event key = (type, place-tier). Confidence = noisy-OR over distinct channels' weights within an 8-min window; a channel that posts the same fact within 15 s of another (aggregator echo, e.g. kyiv_nebo after UI/AR) counts as a partial, not a full, second source. URGENT immediately from one channel with w ≥ 0.8; lower-weight single source → WATCH, promoted on a second channel or reply-chain progression. Cooldowns per tier (URGENT 5 min, WATCH 10, INFO 20) gate *sounds*; body updates are unlimited but coalesced (replace-in-place via a stable ntfy tag per event). A weightless story-30 report joins its zone but does not keep the event alive. A drone reported in the ring and then over the house again re-sounds: a return is news, gated by the tier cooldown like any other sound (#16). All-clear: INFO once after ≥ 10 min without HOME/NEARBY reports AND (a clear message OR Kyiv siren ended).

**Sound budget.** One floor under everything that bypasses Do-Not-Disturb: `urgent_floor_min` (2) is the minimum gap between URGENT *repeats* from any events, so a missile and a ballistic event cannot ring 18 s apart for one raid. A NEW event or a promotion is news and never floored. Per-event and per-(zone, tier) clocks stay as they are; the floor sits under all of them (#16).
...
- 19 Aug 20:50–21:16: URGENT at 20:52:25; Antonov/Святошин appear in the body; ≤ 8 sounds *per event* (six on the ballistic event, three on the Zircon event, nine audible pushes in the window — the criterion is per event, silent INFO never counts); non-Kyiv `Ціль на Ромни!` produces nothing above a log entry; Zircons produce missile WATCH/URGENT.
