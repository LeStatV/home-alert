"""sim_kn.py — copy of sim.py + @kyiv_nebo (KN=0 to exclude, KNTYPE=1 = treat KN bare places as drone).
...
CH = {"kyiv_nebo": ("KN", 0.7), "kpszsu": ("PS", 1.0), "war_monitor": ("WM", 0.9), "nebo_raketa": ("NR", 0.8),
...
PLACES = {  # stem regex -> canonical
    ...
    r"білогородк": "Білогородка",
    # --- added for kyiv_nebo wording (sim_kn only) ---
    r"солом": "Солом'янка", r"почайн": "Почайна", r"видубич": "Видубичі", r"тец-?5": "ТЕЦ-5", r"тец-?6": "ТЕЦ-6",
    r"козин": "Козин", r"вишеньк": "Вишеньки", r"макарів": "Макарів", r"ворзел": "Ворзель", r"конча": "Конча-Заспа",
    r"трипілл": "Трипілля", r"\bцентр\b": "Центр", r"лівий берег|лівобереж": "Лівий берег", r"правий берег": "Правий берег",
    r"анонов": "Антонов", r"сирец|сирц": "Сирець", r"берков": "Берковець", r"білич": "Біличі", r"академ": "Академмістечко", r"гостомел": "Гостомель", r"рембаз": "Рембаза", r"галаган": "Галагани", r"київ|киев|столиц|над містом": "Київ",
}
KYIV_CITY = {"Оболонь", "Мінський масив", "Пуща-Водиця", "Виноградар", "Куренівка", "Поділ", "Троєщина", "Нивки",
             ...
             "Солом'янка", "Почайна", "Видубичі", "ТЕЦ-5", "ТЕЦ-6", "Центр", "Лівий берег", "Правий берег"}
...
def load(d0, d1):
    ms = []
    for ch in CH:
        if ch == "kyiv_nebo" and os.environ.get("KN", "1") == "0": continue
...
def run(d0, d1):
    ...
    for m in ms:
        ...
        kn_bare = os.environ.get("KNTYPE") == "1" and ch == "kyiv_nebo" and pl and not BALLISTIC_WORD.search(ctx)
        if len(text) <= 140 and (DRONE.search(text) or (pl and DRONE.search(ctx)) or kn_bare):
            tier = tier_for(pl)
            if tier:
